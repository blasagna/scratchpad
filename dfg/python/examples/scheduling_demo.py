...
def ordering_blueprint(trace: list, *, side_priority: int = 0) -> GraphSpec:
    ...
    builder.connect("chain3.output", "join.a")
    builder.connect("side.output", "join.b")
    builder.add_input("source", "head.input")
    builder.add_output("merged", "join.output")
...
def readiness_blueprint(rule: ReadinessRule) -> GraphSpec:
    ...
    builder.connect("head.output", "thin.input")
    builder.connect("head.output", "side.input")
    builder.connect("thin.output", "join.a")
    builder.connect("side.output", "join.b")
    builder.add_input("source", "head.input")
    builder.add_output("merged", "join.output")
    return builder.build()
