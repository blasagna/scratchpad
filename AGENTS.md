...
self-contained area with its own nested `AGENTS.md` (loaded on demand when you
...
| Area | What | Details |
|------|------|---------|
| `c_little_book/` | C exercises from the little book of C (Bazel) | [`c_little_book/AGENTS.md`](c_little_book/AGENTS.md) |
| `algo_little_book/` | Algorithm/data-structure exercises in Python (pixi) | [`algo_little_book/AGENTS.md`](algo_little_book/AGENTS.md) |
| `copy_file/` | A `cp`-like file copier, ported to C / C++ / Rust | [`copy_file/AGENTS.md`](copy_file/AGENTS.md) |
| `text_analyzer/` | A text-stats CLI, ported to C / C++ / Rust with cross-port parity | [`text_analyzer/AGENTS.md`](text_analyzer/AGENTS.md) |
| `simple_logger/` | A timestamped log-appender CLI, ported to C / C++ / Rust | [`simple_logger/AGENTS.md`](simple_logger/AGENTS.md) |
| `matrix_ops/` | A 2D matrix arithmetic CLI, ported to C / C++ / Rust, benchmarked against Eigen, xtensor, faer, and nalgebra | [`matrix_ops/AGENTS.md`](matrix_ops/AGENTS.md) |
...
| `dfg/` | A dataflow graph framework for real-time and batch processing — a language-independent design contract plus a Python port (pixi) | [`dfg/AGENTS.md`](dfg/AGENTS.md) |
| `microbit_v2_zephyr/` | A Zephyr RTOS application for the BBC micro:bit V2 — sensors, buzzer, on-device FFT, and BLE notifications (west), plus host-side programs in their own pixi env — a BLE throughput reader, a rerun visualizer, and a tone sweep that checks the reported peak frequency over the console shell | [`microbit_v2_zephyr/AGENTS.md`](microbit_v2_zephyr/AGENTS.md) |
| `rpi_pico_rust_debug/` | A minimal embassy `no_std` firmware for the Raspberry Pi Pico W (RP2040), for exercising `probe-rs` debugging — breakpoints, watchpoints, panic backtraces — via a deliberate bug | [`rpi_pico_rust_debug/AGENTS.md`](rpi_pico_rust_debug/AGENTS.md) |
| `rpi_pico_zephyr_debug/` | The C++/Zephyr counterpart of `rpi_pico_rust_debug` for the same Pico W: the same deliberate ring-buffer bug, for exercising OpenOCD + Raspberry Pi Debug Probe debugging (breakpoints, watchpoints, fatal-error backtraces) via `west`, plus a `native_sim` ztest suite for the pure conversion | [`rpi_pico_zephyr_debug/AGENTS.md`](rpi_pico_zephyr_debug/AGENTS.md) |
| `memory_optimization/` | C++ demos of the cache/memory optimization techniques in Section 6 of Drepper's "What Every Programmer Should Know About Memory" — one small project per technique (Bazel + Google Benchmark; NUMA excluded) | [`memory_optimization/AGENTS.md`](memory_optimization/AGENTS.md) |
| `feather_sense_zephyr/` | A Zephyr RTOS application for the Adafruit Feather Sense (nRF52840) — IMU (an out-of-tree LSM6DS33/LSM6DS3TR-C driver over ST's vendored stmemsc code), magnetometer, environmental, battery and button streams over both USB CDC and BLE, with an RPC channel and a NeoPixel battery indicator (west), plus `native_sim` ztests and host-side rate CLIs and a rerun viewer in their own pixi env | [`feather_sense_zephyr/AGENTS.md`](feather_sense_zephyr/AGENTS.md) |
...
[`matrix_ops/AGENTS.md`](matrix_ops/AGENTS.md).
...
[`memory_optimization/AGENTS.md`](memory_optimization/AGENTS.md).
...
See the per-area `AGENTS.md` files.
...
see [`dfg/AGENTS.md`](dfg/AGENTS.md). Because those dependencies are absent from the
