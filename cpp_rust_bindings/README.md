...
error) then match byte for byte. Three things are outside that guarantee, and
all three are argument-parsing or encoding conventions rather than exprkit
behavior:
...
- **`--help` output.** The C++ CLI hand-rolls its usage text; the Rust one is
  ...
  `statkit` lets each runtime word its own OS errors. (The *unknown option*
  message is shared, though — both print the same two-line hint rather than a
  usage block, precisely so that path stays identical.)
- **A bare `--`.** clap treats it as an end-of-options separator; the C++ CLI,
  which has no such concept, reports it as an unknown option.
...
- **clap and a hand-rolled C parser do not agree for free.** `allow_hyphen_values`
  is required so `-2 ^ 2` is arithmetic rather than an unknown flag, but it also
  stops clap recognizing *any* option after the first positional — so
  `exprkit '1+1' --names` silently loses the flag. `split_options` in
  `rust/src/main.rs` puts C-style semantics back over what clap leaves behind.
