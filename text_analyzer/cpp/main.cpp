...
void print_help() {
    std::cout << kUsage
              << "\n"
                 ...
                 "\n"
                 "Options:\n";
    std::cout << "  --top-n N           number of top words/chars to report (default: "
              << text_analyzer::kDefaultTopN << ")\n";
    std::cout << "  --max-word-len N    max characters per word before truncation (default: "
              << text_analyzer::kDefaultMaxWordLen << ")\n";
    std::cout << "  --word-table-cap N  initial word frequency table capacity (default: "
              << text_analyzer::kDefaultWordTableCap << ")\n";
    std::cout << "  --json              print the summary as JSON instead of text\n"
...
int main(int argc, char* argv[]) {
    ...
    auto set_int_flag = [&opts](std::string_view name, unsigned int value) {
        if (name == "--top-n") opts.config.top_n = value;
        else if (name == "--max-word-len") opts.config.max_word_len = value;
        else if (name == "--word-table-cap") opts.config.word_table_init_cap = value;
    };
    ...
    for (std::size_t i = 1; i < args.size(); i++) {
        ...
        if (arg == "--top-n" || arg == "--max-word-len" || arg == "--word-table-cap") {
            if (i + 1 >= args.size()) {
        ...
        if (const std::size_t eq = arg.find('=');
            ...
            if (name == "--top-n" || name == "--max-word-len" || name == "--word-table-cap") {
                const std::optional<unsigned int> n = parse_positive(value);
                if (!n) {
    ...
    const text_analyzer::TextStats stats = text_analyzer::analyze(in, opts.config);
    if (opts.json) {
