...
struct AnalyzerConfig {
    ...
    unsigned int word_table_init_cap = kDefaultWordTableCap;  // reserve hint for the word table
};
...
//         well-defined results (e.g. no words kept). The CLI validates user input.
TextStats analyze(std::istream& in, const AnalyzerConfig& config = {});

// print_stats - writes a formatted summary of stats to os: lines/words/characters
// totals followed by ranked lists of the top words and top characters, each with
// its count and a percentage of the relevant total.
