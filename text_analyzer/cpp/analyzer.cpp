...
TextStats analyze(std::istream& in, const AnalyzerConfig& config) {
    ...
    while (in.get(c)) {
        ...
        if (std::isalpha(uc)) {
            ...
            if (std::ssize(word) < static_cast<std::ptrdiff_t>(config.max_word_len) - 1) {
    ...
    for (char ch = kPrintableAsciiMin; ch <= kPrintableAsciiMax; ch++) {
        ...
        if (count > 0) stats.top_chars.push_back({ch, count});
    }
...
void print_stats(std::ostream& os, const TextStats& stats) {
    ...
    for (std::size_t i = 0; i < stats.top_words.size(); i++) {
        const double pct = stats.word_count > 0
            ? 100.0 * static_cast<double>(stats.top_words[i].count) / static_cast<double>(stats.word_count)
            : 0.0;
        os << std::format("  {}. {} ({}, {:.1f}%)\n", i + 1,
    ...
    for (std::size_t i = 0; i < stats.top_chars.size(); i++) {
        const double pct = stats.char_count > 0
            ? 100.0 * static_cast<double>(stats.top_chars[i].count) / static_cast<double>(stats.char_count)
            : 0.0;
        os << std::format("  {}. '{}' ({}, {:.1f}%)\n", i + 1,
...
void print_stats_json(std::ostream& os, const TextStats& stats) {
    ...
    for (std::size_t i = 0; i < stats.top_words.size(); i++) {
        const double freq = stats.word_count > 0
            ? static_cast<double>(stats.top_words[i].count) / static_cast<double>(stats.word_count)
            : 0.0;
        os << (i == 0 ? "" : ",");
    ...
    for (std::size_t i = 0; i < stats.top_chars.size(); i++) {
        const double freq = stats.char_count > 0
            ? static_cast<double>(stats.top_chars[i].count) / static_cast<double>(stats.char_count)
            : 0.0;
        os << (i == 0 ? "" : ",");
        os << "\n    {\"char\": \"";
