...
- **This area builds with west/CMake** — not Bazel, pixi, or cargo — and is a
  *freestanding* Zephyr application outside `~/zephyrproject`, so `ZEPHYR_BASE`
...
- **`build/` holds symlinks pointing outside the repo, into `~/zephyrproject`** — a glob
  ...
  edit the Zephyr installation in place. See the root `AGENTS.md`.
