...
1. Sample the ambient temperature continuously at 1 Hz
1. Capture press and release events for both the A and B buttons
1. When the touch-sensitive logo is pressed, capture audio for 3 seconds. Calculate the peak frequency of the audio using an on-device FFT computation. Expect tones in the range of human hearing. Print the frequency to the LED matrix display.
1. Make the device a BLE peripheral.
  1. Expose a characteristic to stream the accelerometer data in fixed-point values over indications for high throughput. Batch the samples to maximize throughput over each connection interval.
